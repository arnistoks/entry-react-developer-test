...
const Category = () => {
    ...
    function DisplayProducts() {
        const {loading, error, data} = useQuery(getProducts(category));
        if (loading) return <p>Loading...</p>;
        if (error) return <p>Error :(</p>;
        console.log(data)

        return data.category.products.map((product) => (
            <div className="containerProduct" key={product.id} onClick={() => navigate(`/product/${product.id}`)}>
               <div className="imageContainer">
                   <img className="imageProduct" src={product.gallery[0]} alt={product.name}/>
                   {!product.inStock && <p className="overlayText">OUT OF STOCK</p>}
               ...
               <p className="nameProduct">{`${product.brand} ${product.name}`}</p>
               <p className="priceProduct">{`${moneySymbol}${product.prices[currentAmount()].amount.toFixed(2)}`}</p>
                {!product.inStock && <div className="overlay" />}
            </div>
        ))
    }
    ...
    return (
        <section className="categorySection">
            <h1 className="titleProducts">{category.toUpperCase()}</h1>
